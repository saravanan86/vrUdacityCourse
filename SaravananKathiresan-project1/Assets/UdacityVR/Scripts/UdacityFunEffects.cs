...
public class UdacityFunEffects : MonoBehaviour {
	...
	private bool sizeDoubled = false;
	...
	public void ResizeShape(){
		...
		if (initSize == Vector3.zero) {
			initSize = activeShape.transform.localScale;
		}

		if (!sizeDoubled) {
			activeShape.transform.localScale = new Vector3 (2f, 2f, activeShape.transform.localScale.y);
			sizeDoubled = true;
		} else {
			activeShape.transform.localScale = initSize;
			sizeDoubled = false;
		}
    ...
    {
		Vector3 currentScale = activeShape.transform.localScale;
        if (cube.gameObject.activeInHierarchy)
        {
		...
		activeShape.transform.localScale = currentScale;
    }
